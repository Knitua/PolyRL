...
from PolyRL.models import adapt_state_dict, models, register_model
from PolyRL.rl_env import generate_complete_smiles, TokenEnv
from PolyRL.scoring_functions import (
    custom_scoring_functions,
    register_custom_scoring_function,
    Task,
)
from PolyRL.vocabulary import Vocabulary
...
def run_reinvent(cfg, task):
    ...
    # ➕ 加载你自己微调的权重（注意这一步是替换核心部分）
    #custom_weights = torch.load("/root/autodl-tmp/PolyRL-open-main/PolyRL/priors/finetuned_gpt2.pt", map_location=device)
    #actor_inference.module[0].module[0].module.feature_extractor.load_state_dict(custom_weights, strict=False)
    #actor_training.module[0].module[0].module.feature_extractor.load_state_dict(custom_weights, strict=False)
    ...
    while not task.finished:
        ...
        # 如果你data里有reward，可以这样做
        reward = data.get(("next", "reward")).squeeze(-1).sum(-1)
        valid_mask = (reward >= 0).float()

        # 然后再
        loss_p = -(1 / (agent_likelihood + 1e-8))
        loss_p = loss_p * valid_mask
        loss += 5 * 1e3 * loss_p.mean()'''
